...
UTILITIES_METERING_SPECS = specs_from_resource_specs(UTILITIES_METERING_RESOURCE_SPECS)


@pytest.mark.parametrize(
    "spec",
    UTILITIES_METERING_SPECS,
    ids=lambda spec: spec["name"],
)
def test_utilities_metering_models_are_permissive_and_include_sample_fields(spec):
...
def test_utilities_metering_resources_are_attached_with_expected_path_and_model(spec):
...
def test_utilities_metering_resource_methods_are_non_destructive_mock_calls(spec):
...
def test_utilities_metering_models_are_top_level_exports():
    for spec in UTILITIES_METERING_SPECS:
