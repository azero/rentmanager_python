...
COMMUNICATIONS_DOCUMENTS_PORTALS_WEB_SPECS = specs_from_resource_specs(
    COMMUNICATIONS_DOCUMENTS_PORTALS_WEB_RESOURCE_SPECS
)
...
@pytest.mark.parametrize(
    "spec",
    COMMUNICATIONS_DOCUMENTS_PORTALS_WEB_SPECS,
    ids=lambda spec: spec["name"],
)
def test_communications_documents_portals_web_models_are_permissive_and_include_sample_fields(spec):
...
def test_communications_documents_portals_web_resources_are_attached_with_expected_path_and_model(spec):
...
def test_communications_documents_portals_web_resource_methods_are_non_destructive_mock_calls(spec):
...
def test_communications_documents_portals_web_registry_includes_all_live_help_endpoints():
    registry_paths = {(endpoint.method, endpoint.path) for endpoint in EndpointRegistry.default()}
    missing_paths = [
        (spec["name"], method, path)
        for spec in COMMUNICATIONS_DOCUMENTS_PORTALS_WEB_SPECS
...
def test_communications_documents_portals_web_models_are_top_level_exports():
    for spec in COMMUNICATIONS_DOCUMENTS_PORTALS_WEB_SPECS:
