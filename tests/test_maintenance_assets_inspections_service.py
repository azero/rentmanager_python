...
MAINTENANCE_ASSETS_INSPECTIONS_SERVICE_SPECS = specs_from_resource_specs(
    MAINTENANCE_ASSETS_INSPECTIONS_SERVICE_RESOURCE_SPECS
)
...
@pytest.mark.parametrize(
    "spec",
    MAINTENANCE_ASSETS_INSPECTIONS_SERVICE_SPECS,
    ids=lambda spec: spec["name"],
)
def test_maintenance_assets_inspections_service_models_are_permissive_and_include_sample_fields(spec):
...
def test_maintenance_assets_inspections_service_resources_are_attached_with_expected_path_and_model(spec):
...
def test_maintenance_assets_inspections_service_resource_methods_are_non_destructive_mock_calls(spec):
...
def test_maintenance_assets_inspections_service_registry_includes_all_live_help_endpoints():
    registry_paths = {(endpoint.method, endpoint.path) for endpoint in EndpointRegistry.default()}
    missing_paths = [
        (spec["name"], method, path)
        for spec in MAINTENANCE_ASSETS_INSPECTIONS_SERVICE_SPECS
...
def test_maintenance_assets_inspections_service_models_are_top_level_exports():
    for spec in MAINTENANCE_ASSETS_INSPECTIONS_SERVICE_SPECS:
