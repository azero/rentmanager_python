...
from rentmanager_api.accounting import ACCOUNTING_RESOURCE_SPECS
...
ACCOUNTING_SPECS = specs_from_resource_specs(ACCOUNTING_RESOURCE_SPECS)


@pytest.mark.parametrize("spec", ACCOUNTING_SPECS, ids=lambda spec: spec["name"])
def test_accounting_todo_models_are_permissive_and_include_sample_fields(spec):
    model = getattr(models, spec["model_name"])

    for field in spec["fields"]:
        assert field in model.model_fields

    instance = model.model_validate({spec["fields"][0]: 123, "UnexpectedField": "kept"})
    assert getattr(instance, "UnexpectedField") == "kept"


@pytest.mark.parametrize("spec", ACCOUNTING_SPECS, ids=lambda spec: spec["name"])
def test_accounting_todo_resources_are_attached_with_expected_path_and_model(spec):
...
@pytest.mark.parametrize("spec", ACCOUNTING_SPECS, ids=lambda spec: spec["name"])
def test_accounting_todo_resource_methods_are_non_destructive_mock_calls(spec):
...
def test_accounting_todo_models_are_top_level_exports():
    for spec in ACCOUNTING_SPECS:
