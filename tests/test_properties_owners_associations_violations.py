...
from rentmanager_api.properties_owners import PROPERTIES_OWNERS_ASSOCIATIONS_VIOLATIONS_RESOURCE_SPECS
...
PROPERTIES_OWNERS_ASSOCIATIONS_VIOLATIONS_SPECS = specs_from_resource_specs(
    PROPERTIES_OWNERS_ASSOCIATIONS_VIOLATIONS_RESOURCE_SPECS
)


@pytest.mark.parametrize("spec", PROPERTIES_OWNERS_ASSOCIATIONS_VIOLATIONS_SPECS, ids=lambda spec: spec["name"])
def test_properties_owners_associations_violations_models_are_permissive_and_include_sample_fields(spec):
...
@pytest.mark.parametrize("spec", PROPERTIES_OWNERS_ASSOCIATIONS_VIOLATIONS_SPECS, ids=lambda spec: spec["name"])
def test_properties_owners_associations_violations_resources_are_attached_with_expected_path_and_model(spec):
...
@pytest.mark.parametrize("spec", PROPERTIES_OWNERS_ASSOCIATIONS_VIOLATIONS_SPECS, ids=lambda spec: spec["name"])
def test_properties_owners_associations_violations_resource_methods_are_non_destructive_mock_calls(spec):
...
def test_properties_owners_associations_violations_registry_includes_all_live_help_endpoints():
    registry_paths = {(endpoint.method, endpoint.path) for endpoint in EndpointRegistry.default()}
    missing_paths = [
        (spec["name"], method, path)
        for spec in PROPERTIES_OWNERS_ASSOCIATIONS_VIOLATIONS_SPECS
...
def test_properties_owners_associations_violations_models_are_top_level_exports():
    for spec in PROPERTIES_OWNERS_ASSOCIATIONS_VIOLATIONS_SPECS:
