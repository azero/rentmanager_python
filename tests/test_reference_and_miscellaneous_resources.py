...
from rentmanager_api.reference_misc import REFERENCE_MISC_RESOURCE_SPECS
...
REFERENCE_MISC_SPECS = specs_from_resource_specs(REFERENCE_MISC_RESOURCE_SPECS)


@pytest.mark.parametrize("spec", REFERENCE_MISC_SPECS, ids=lambda spec: spec["name"])
def test_reference_misc_models_are_permissive_and_include_sample_fields(spec):
...
@pytest.mark.parametrize("spec", REFERENCE_MISC_SPECS, ids=lambda spec: spec["name"])
def test_reference_misc_resources_are_attached_with_expected_path_and_model(spec):
...
@pytest.mark.parametrize("spec", REFERENCE_MISC_SPECS, ids=lambda spec: spec["name"])
def test_reference_misc_resource_methods_are_non_destructive_mock_calls(spec):
...
def test_reference_misc_models_are_top_level_exports():
    for spec in REFERENCE_MISC_SPECS:
