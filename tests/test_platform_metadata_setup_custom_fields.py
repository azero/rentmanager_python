...
from rentmanager_api.platform_metadata import PLATFORM_METADATA_RESOURCE_SPECS
...
def concrete_get_value(path_template: str) -> str | int:
    return "sample-token" if "{token}" in path_template else 123


def concrete_get_path(path_template: str) -> str:
    return path_template.replace("{token}", "sample-token").replace("{id}", "123")


PLATFORM_METADATA_SPECS = specs_from_resource_specs(PLATFORM_METADATA_RESOURCE_SPECS)


@pytest.mark.parametrize("spec", PLATFORM_METADATA_SPECS, ids=lambda spec: spec["name"])
def test_platform_metadata_models_are_permissive_and_include_sample_fields(spec):
...
@pytest.mark.parametrize("spec", PLATFORM_METADATA_SPECS, ids=lambda spec: spec["name"])
def test_platform_metadata_resources_are_attached_with_expected_path_and_model(spec):
...
@pytest.mark.parametrize("spec", PLATFORM_METADATA_SPECS, ids=lambda spec: spec["name"])
def test_platform_metadata_resource_methods_are_non_destructive_mock_calls(spec):
    ...
    get_paths = sorted(path for method, path in endpoint_paths if method == "GET" and path != f"/{spec['name']}")
    for path in get_paths:
        resource.get(concrete_get_value(path))
        expected_calls.append(("GET", concrete_get_path(path), None))
...
def test_platform_metadata_models_are_top_level_exports():
    for spec in PLATFORM_METADATA_SPECS:
